...
namespace Portable.Drawing.Imaging.ImageFormats
{
    public class JpegRgbToYCbCrConverter
    {
        ...
        public void ConvertRgba32ToYCbCr8(ReadOnlySpan<byte> rgb, Span<byte> ycbcr, int count)
        {
            ...
            {
                a = Unsafe.Add(ref sourceRef, 3);
                r = Unsafe.Add(ref sourceRef, 2);
                g = Unsafe.Add(ref sourceRef, 1);
                b = Unsafe.Add(ref sourceRef, 0);

                if (a < 128) // threshold transparent to white
                {
                    r = 255; g=255; b=255;
                }
