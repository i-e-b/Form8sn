...
namespace Portable.Drawing.Imaging.ImageFormats
{
    public class PortableImage : MarshalByRefObject, ICloneable, IToolkitImage
    {
        ...
        public PortableImage()
        {
            ...
            NumFrames = 0;
            frames = new[]{new Frame(this, 0, 0, PixelFormat.Undefined)};
            LoadFormat = null;
            Palette = null;
            TransparentPixel = -1;
        }
        ...
        public PortableImage(int width, int height, PixelFormat pixelFormat)
        {
            ...
            NumFrames = 0;
            frames = new[]{new Frame(this, width, height, pixelFormat)};
            LoadFormat = Memory;
            Palette = null;
            TransparentPixel = -1;
        }
        ...
        public Frame? Frame()
        {
            return GetFrame(_activeFrame);
        }
        
        public void SetActiveFrame(int frame)
        {
            if (frames == null) throw new Exception("This image has no frames");
            if (frame < 0 || frame >= frames.Length) throw new Exception("Invalid frame index for this image");
            _activeFrame = frame;
        }

        public void SetFrame(int frame, Frame? newFrame)
        {
            if (frames == null || frame < 0 || frame >= NumFrames || newFrame == null) return;
